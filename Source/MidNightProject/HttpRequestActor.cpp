...
void AHttpRequestActor::PostRequset(const FString url)
{
	...
	//GEngine->AddOnScreenDebugMessage(-1, 3.0, FColor::Yellow, FText::AsNumber(instrumentid).ToString());
	chatData.Add("id", FText::AsNumber(instrumentid).ToString());
...
void AHttpRequestActor::PostVoideRequset(const FString url)
{
	TMap<FString, FString> chatData;
...
void AHttpRequestActor::OnPostData(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnectedSuccessfully)
{
	if (bConnectedSuccessfully)
	{
		...
		FTimerHandle Timer;
		GetWorld()->GetTimerManager().SetTimer(Timer,this ,&AHttpRequestActor::SetMouseDisable, 10.0f, false);
	}
...
void AHttpRequestActor::SetMouseDisable()
{
	APlayerController* PlayerC = Cast<APlayerController>(player->GetController());
	if (PlayerC != nullptr)
	{
		PlayerC->SetShowMouseCursor(false);
		player->bSearching = false;
	}
}
