...
void UJsonParseLibrary::PlayPCMData(const TArray<uint8>& PCMData, class AMidNightProjectCharacter* player)
{
	...
	FString headerFilePath = FPaths::ProjectPersistentDownloadDir() + "/WaveHeader.raw";
	FFileHelper::LoadFileToArray(headerData, *headerFilePath);
	ByteData.AddUninitialized(headerData.Num());
	FMemory::Memcpy(ByteData.GetData(), headerData.GetData(), headerData.Num());*/

	// 본문 추가
	ByteData.AddUninitialized(PCMData.Num() * sizeof(uint16));
	FMemory::Memcpy(ByteData.GetData() + 45, PCMData.GetData(), PCMData.Num());
	SoundWave->QueueAudio(ByteData.GetData(), ByteData.Num());


	// 받은 데이터를 wav 파일로 저장4141
	FString saveURL = FPaths::ProjectPersistentDownloadDir() + FString("/MyWave.mp3");
...
void UJsonParseLibrary::SaveHeader(const TArray<uint8>& sourceData, const FString savePath)
{
	int32 cpyNum = 45;

	TArray<uint8> destData;
	destData.AddUninitialized(cpyNum);
	FMemory::Memcpy(destData.GetData(), sourceData.GetData(), cpyNum);

	FString path = FPaths::ProjectPersistentDownloadDir() + FString("/WaveHeader.raw");
	bool isSaved = FFileHelper::SaveArrayToFile(destData, *path);
	if (isSaved)
	{
		for (uint8 byte : destData)
		{
			UE_LOG(LogTemp, Warning, TEXT("%d"), byte);
		}
	}

	UE_LOG(LogTemp, Warning, TEXT("Result: %s"), isSaved ? *FString("Header Saved!") : *FString("Header Failed..."));
}
